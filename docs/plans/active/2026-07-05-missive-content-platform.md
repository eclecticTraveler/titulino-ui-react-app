...
- [ ] T48 — Manual verification: point a low-volume job (e.g. `birthdays`) at a different existing template name via the new UI, run it manually, confirm the *new* template's content is what actually sends; then revert. **DB deployed 2026-07-16** — no longer blocked on sqitch. Still blocked on the C# code (`MessageManager.cs`'s `ResolveJobTemplateName`) actually running on `pd-titulino-lang` — needs the updated `titulino-communication` build published to the server first.
...
- [ ] Manual verification: **DB deployed 2026-07-16**, no longer blocked on sqitch. Still needs the updated `titulino-communication` code published to `pd-titulino-lang` first. Then: toggle a low-volume job off, trigger it manually, confirm it logs "skipping execution" and sends nothing; toggle back on, confirm it sends normally.
