...
**Call 1:**
- **New theme or existing theme?** — Is this a brand-new course slug being added for the first time, or an additional run (new courseCodeId) for a theme whose nav items and configs already exist?
  - *New theme* — slug does not yet exist anywhere; all phases run
  - *Existing theme* — nav items, submenu files, i18n keys already exist; only `course-theme-registry.data.json` and spine data are updated
- **Theme name** — kebab-case slug (e.g. `english-connect`, `meditaciones`). For existing themes this must exactly match the key already in `course-theme-registry.data.json`.
- **courseCodeId** — The new ID from the admin (format: `COURSE_NAME_MON_YEAR_COURSE_NN`). Type "pending" if not created yet.
- **Display names** *(new theme only)* — EN / ES / PT nav label. Leave blank for existing theme.
- **Languages** *(new theme only)* — Which language configs? (English / Spanish / Portuguese — multi-select). Leave blank for existing theme.

> **If existing theme: skip Calls 2 and 3 entirely. Go directly to Step 1.**
...
**If existing theme:** read `src/assets/data/course-theme-registry.data.json` and verify the theme key exists. Show the user the current array (e.g. `["COURSE_01", "COURSE_02"]`) and confirm the new courseCodeId is not already in it. If the theme key is missing, warn the user — they may have typed the wrong slug. Then skip to Step 2.

**If new theme:** read all files below.
...
**If existing theme**, output this shorter plan and wait for confirmation:

```
Existing theme: <theme-name>
Current courseCodeIds: [<existing array from registry>]
Adding: <new courseCodeId>

Files to modify:
  MOD  src/assets/data/course-theme-registry.data.json  (append courseCodeId to existing array)

Spine data entries to generate (manual fill required):
  after-purchase-messages.json
  welcome-messages.json
  chapter-class-data.json
  grammar-class-data.json
  chapter-book-data.json
  pdf-data.json
  quizlet-pratice-data.json
  know-me-survey-data.json
  speaking-practice-data.json
  course-progress-data.json

Proceed?
```

After confirmation: run Phase 1 (registry append only), skip Phases 2–4c, run Phase 7, run Phase 8.

---

**If new theme**, output the full plan below and wait for confirmation:
...
> **If existing theme:** only perform the `course-theme-registry.data.json` append below — append `"<courseCodeId>"` to the existing array for this theme key. Skip all other Phase 1 changes and skip Phases 2, 3, 4, 4b, and 4c entirely. Resume at Phase 7.

**`src/configs/CourseThemeConfig.js`** *(new theme only)*
...
**`src/services/CentralCourseThemeService.js`** *(new theme only)*
Add to `levelMapping`:
```js
...
- *New theme:* add a new key: `"<theme-name>": ["<courseCodeId>"]`. If courseCodeId is "pending", use `"TODO_<THEME_UPPER>_COURSE_01"` as placeholder.
- *Existing theme:* append `"<courseCodeId>"` to the existing array. Example: `["COURSE_01", "COURSE_02"]` → `["COURSE_01", "COURSE_02", "COURSE_03"]`.

**`src/assets/data/badge-theme-registry.data.json`** *(new theme only, and only if tiers selected)*
Add entry:
```json
"<theme-name>": {
    "Participation": {
...
### Phase 3 — Submenu files *(new theme only — skip if existing theme)*
...
### Phase 4 — Top-level navigation config *(new theme only — skip if existing theme)*
...
### Phase 4b — Search keywords (`SearchAssociation.js`) *(new theme only — skip if existing theme)*
...
### Phase 4c — Wire Spanish and Portuguese to English capabilities *(new theme only — skip if existing theme)*
