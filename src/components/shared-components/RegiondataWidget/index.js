...
const getMapFeatures = (mapSource, mapType) => {
  ...
  if (mapSource.type === "Topology" && mapSource.objects) {
    ...
    const topologyObject = mapSource.objects[preferredObjectKey];
    const converted = topojsonFeature(mapSource, topologyObject);
...
const getFeatureRegionName = (geoFeature, mapType) => {
  ...
  return props.NAME_1 || props.name || props.NAME || props.ADMIN || "";
};
    ...
    <div style={{ transform: mapType === "world" ? `translateY(${worldVerticalOffset}px)` : "none" }}>
      ...
      >
        {features.map((geoFeature, index) => {
          ...
          const fill = isHovered ? getRegionHoverColor(geoName, data) : getHighlightedRegion(geoName, data);
...
RegiondataWidget.propTypes = {
  ...
  mapSource: PropTypes.oneOfType([PropTypes.object, PropTypes.array]),
